<template>
  <div class="space-y-6">
    <!-- Поиск по TG ID -->
    <div :class="['rounded-lg border p-6', isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-gray-100 border-gray-300']">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="flex items-end">
          <button
            @click="searchUser"
            :class="[
              'w-full px-4 py-2 font-medium rounded hover:opacity-90 transition',
        ...
        <div class="flex items-end">
          <button
            @click="loadAllUsers"
            :class="[
              'w-full px-4 py-2 rounded font-medium hover:opacity-90 transition',
              isDarkMode
      ...
      <div class="mt-4 flex gap-2 flex-wrap">
        ...
        <button
          @click="exportToCSV"
          :class="[
            'px-4 py-2 rounded font-medium transition ml-auto',
            isDarkMode
    ...
    <div :class="['rounded-lg border overflow-hidden', isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-300']">
      ...
      <table v-else class="w-full">
        <thead>
          <tr :class="['border-b', isDarkMode ? 'border-gray-800 bg-gray-800' : 'border-gray-300 bg-gray-200']">
            <th :class="['px-6 py-3 text-left text-sm font-semibold', isDarkMode ? 'text-gray-300' : 'text-gray-700']">
              TG ID
            </th>
            <th :class="['px-6 py-3 text-left text-sm font-semibold', isDarkMode ? 'text-gray-300' : 'text-gray-700']">
              Username
            ...
            <th :class="['px-6 py-3 text-right text-sm font-semibold', isDarkMode ? 'text-gray-300' : 'text-gray-700']">
              Действия
            </th>
          </tr>
        </thead>
        <tbody :class="['divide-y', isDarkMode ? 'divide-gray-800' : 'divide-gray-300']">
          ...
          >
            <td :class="['px-6 py-4 text-sm font-mono', isDarkMode ? 'text-white' : 'text-black']">
              {{ user.tg_id }}
            </td>
            <td :class="['px-6 py-4 text-sm', isDarkMode ? 'text-white' : 'text-black']">
              {{ user.username || '-' }}
            </td>
            <td :class="['px-6 py-4 text-sm', isDarkMode ? 'text-white' : 'text-black']">
              {{ user.first_name || '-' }}
            ...
            <td :class="['px-6 py-4 text-sm', isDarkMode ? 'text-gray-400' : 'text-gray-600']">
            ...
            <td class="px-6 py-4 text-sm text-right space-x-2">
              <button
                ...
                :class="[
                  'px-3 py-1 rounded font-medium text-xs transition',
                  user.is_premium
      ...
      <!-- Пагинация -->
      <div v-if="filteredUsers.length > itemsPerPage" :class="['flex items-center justify-between px-6 py-4 border-t', isDarkMode ? 'border-gray-800 bg-gray-800' : 'border-gray-300 bg-gray-100']">
        <button
          @click="currentPage--"
